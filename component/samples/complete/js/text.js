...
function _text(mode, x, y, w, h) {
	...
	this.paint = (gr) => {
		if (this.mode === 'lastfm_bio' && !lastfm.api_key.length) {
			gr.GdiDrawText('Use the right click menu to set your Last.fm API key.', panel.fonts.normal, panel.colours.text, this.x, this.y + _scale(12), this.w, panel.row_height, LEFT);
			return;
		}
	...
	this.init = () => {
		...
		case 'lastfm_bio':
			this.get = function () {
				if (lastfm.api_key.length == 0 || !_tagged(this.artist))
					return;
