...
namespace mozjs
{
	...
	JsFbProfiler::JsFbProfiler(JSContext* cx, const std::string& name) : pJsCtx_(cx), name_(name)
	{
		timer_.start();
	}

	std::unique_ptr<JsFbProfiler> JsFbProfiler::CreateNative(JSContext* cx, const std::string& name)
	{
