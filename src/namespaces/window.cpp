...
namespace mozjs
{
	...
	bool Window::get_IsTransparent()
	{
		...
		return m_parent.supportsTransparency_;
	}
	...
	int32_t Window::get_MaxHeight()
	{
		...
		return m_parent.maxSize_.y;
	}

	int32_t Window::get_MaxWidth()
	{
		if (m_isFinalized)
		{
			return 0;
	...
	int32_t Window::get_MinHeight()
	{
		...
		return m_parent.minSize_.y;
	}

	int32_t Window::get_MinWidth()
	{
		if (m_isFinalized)
		{
			return 0;
	...
	void Window::put_MaxHeight(int32_t height)
	{
		if (m_isFinalized)
		{
			return;
		...
		m_parent.maxSize_.y = height;
		m_parent.NotifySizeLimitChanged();
	}

	void Window::put_MaxWidth(int32_t width)
	{
		if (m_isFinalized)
		{
			return;
		...
		m_parent.maxSize_.x = width;
		m_parent.NotifySizeLimitChanged();
	}

	void Window::put_MinHeight(int32_t height)
	{
		if (m_isFinalized)
		{
			return;
		...
		m_parent.minSize_.y = height;
		m_parent.NotifySizeLimitChanged();
	}

	void Window::put_MinWidth(int32_t width)
	{
		if (m_isFinalized)
		{
			return;
