...
namespace smp
{
	...
	Task::Task(EventPriority priority) : taskNumber_(g_currentTaskNumber++), priority_(priority) {}
	...
	RunnableTask::RunnableTask(std::shared_ptr<Runnable> pRunnable, EventPriority priority) : Task(priority), pRunnable_(std::move(pRunnable)) {}
	...
	TaskController::TaskController(std::shared_ptr<PanelTarget> pTarget) : pTarget_(std::move(pTarget)) {}
